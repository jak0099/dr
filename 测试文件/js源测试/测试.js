...
(() => {
    try {
        ...
        // 1. Base64加密地址提取
        const enc = html.match(/var (?:player|play)_url\s*=\s*["']([^"']+)/)[1];
        ...
        let realUrl = atob(decoded);
        
        // 3. 地址标准化
        realUrl = realUrl
            .replace(/\\+/g, '')
            .replace(/^\/\//, 'https://')
            .replace(/(:\d{4,5})\//, (m,p1) => [80,443].includes(p1.slice(1)) ? m : '/');
        
        // 4. 严格保持代码结构
        input = realUrl; // 必须赋值给input
        let url = getProxyUrl() + '&url=' + encodeURIComponent(input);
        
        console.log('最终代理地址:', url); // 调试点
