/**
 * 新版代理增强型M3U8处理器
 * @param {string} url 原始m3u8地址
 * @returns {string} 处理后的内容
 */
function superProxyM3u8(url) {
    // 代理配置
    const proxy = {
        base: 'http://127.0.0.1:2077/proxy?',
        params: {
            target: encodeURIComponent(url),
            realIp: `119.${Math.random()*255|0}.${Math.random()*255|0}.${Math.random()*255|0}`,
            headers: {
    ...
    function genProxy(target) {
        return `${proxy.base}${Object.entries({
            ...proxy.params,
            target: encodeURIComponent(target)
        }).map(([k,v])=>`${k}=${typeof v === 'object' ? encodeURIComponent(JSON.stringify(v)) : v}`).join('&')}`;
    }

    // 递归处理嵌套m3u8
    function process(url) {
        const content = request(genProxy(url));
        if(url.endsWith('.m3u8')) {
            return content.split('\n').map(line => {
                if(line.endsWith('.m3u8')) {
                    return process(urljoin(url, line));
                }
                if(line.endsWith('.ts')) {
                    return genProxy(urljoin(url, line));
                }
                return line;
            }).join('\n');
        }
    ...
    // 执行主处理
    let result = process(url);
    
    // 去广告二次处理
    result = fixAdM3u8(result, url, 'reg:(/ads/|adsegment)');
...
var rule = {
    ...
    lazy: $js.toString(() => {
    const rawUrl = pdfh(html, 'script:contains(m3u8)').match(/http[^\'"]*?m3u8/)[0];
    const final = superProxyM3u8(rawUrl);
    return JSON.stringify({
        parse: 0,
        url: `data:application/vnd.apple.mpegurl;base64,${Base64.encode(final)}`
    });
})
