var rule = {
    ...
    searchUrl: '/vodsearch/**kw**----------fypage---.html',
    searchable: 2,
    quickSearch: 0,
    filterable: 1,
    filter: {
    ...
    filter_url: '{{fl.area}}/{{fl.by}}/{{fl.class}}',
    headers: {
    ...
    play_parse: true,
    double: true,
    推荐: 'ul.swiper-wrapper li;.ewave-vodlist__thumb&&data-original;.ewave-vodlist__thumb&&title;.pic-text&&Text;a&&href',
    一级: 'ul.ewave-vodlist.clearfix li;.ewave-vodlist__thumb&&data-original;.ewave-vodlist__thumb&&title;.pic-text&&Text;a&&href',
    二级: {
        "title": "h1&&Text",
        "img": ".ewave-vodlist__thumb&&data-original",
        "desc": ".data.hidden-sm&&Text;;;.ewave-content__detail p:eq(1) a&&Text;;.ewave-content__detail p:eq(2) a&&Text",
        "content": ".ewave-content__detail p:eq(-1)&&Text",
        "tabs": "ul.nav-tabs.swiper-wrapper li",
        "lists": "ul.ewave-content__playlist:eq(#id) li"
    },
    搜索: 'ul.ewave-vodlist__media.clearfix li;.ewave-vodlist__thumb&&data-original;.ewave-vodlist__thumb&&title;.pic-text&&Text;a&&href',
    lazy: $js.toString(() => {
        let html = request(input);
        let jstr = JSON.parse(html.match(/player_aaaa.*?=(\{.*?\});/)[1]);
        let jxpath = '/bbplayer/api.php';
        let postData = {'vid': jstr.url};
        let data = post(jxpath, postData).data;
        let url;
        
        if (/(\.m3u8|\.mp4)/.test(data.url)) {
            url = data.url;
        } else if (data.urlmode === 1) {
            url = decode1(data.url);
        } else if (data.urlmode === 2) {
            url = decode2(data.url);
        } else if (/(\.m3u8|\.mp4)/.test(jstr.url)) {
            url = jstr.url;
        }
        ...
        function decode1(val) {
            let url = custom_str_decode(val);
            let parts = url.split("/");
            let result = "/".join(parts.slice(2));
            let key1 = JSON.parse(atob(parts[1]));
            let key2 = JSON.parse(atob(parts[0]));
            let decoded = atob(result);
            return de_string(key1, key2, decoded);
        }

        function custom_str_decode(val) {
        ...
        function de_string(key_array, value_array, input_str) {
            let result = "";
            for (let char of input_str) {
                if (/^[a-zA-Z]$/.test(char)) {
                    let index = key_array.indexOf(char);
                    if (index !== -1) {
        ...
        function decode2(url) {
            let key = "PXhw7UT1B0a9kQDKZsjIASmOezxYG4CHo5Jyfg2b8FLpEvRr3WtVnlqMidu6cN";
            url = atob(url);
            let result = "";
            for (let i = 1; i < url.length; i += 3) {
                try {
                    let index = key.indexOf(url[i]);
                    if (index === -1) {
                        result += url[i];
                    } else {
                        result += key[(index + 59) % 62];
                    }
                } catch (e) {
                    break;
                }
