...
/**
 * 增强版正则过滤广告TS，支持多级嵌套路径修复
 * @param {string} m3u8_text - 原始文本或空
 * @param {string} m3u8_url - 基准URL
 * @param {string|RegExp} ad_remove - 正则表达式或特征标记
 * @param {boolean} [strictMode=false] - 是否严格检查EXT标签连续性
 * @returns {string} 净化后的M3U8内容
 */
function fixAdM3u8Pro(m3u8_text, m3u8_url, ad_remove, strictMode = false) {
    // 参数校验强化
    if (!m3u8_text && (!m3u8_url || !/^https?:\/\//.test(m3u8_url))) {
        console.error('Invalid input: Missing valid m3u8 content or URL');
        return '';
    }
    ...
    if (!m3u8_text) {
        try {
            m3u8_text = fetchWithRetry(m3u8_url, 2); // 自定义重试函数
        } catch (e) {
    ...
    // 结构解析容错
    const EXTINF_INDEX = m3u8_text.indexOf('#EXTINF');
    const ENDLIST_INDEX = m3u8_text.indexOf('#EXT-X-ENDLIST');
    if (EXTINF_INDEX === -1 || ENDLIST_INDEX === -1) {
        console.warn('非标准M3U8结构，尝试兼容处理');
        return m3u8_text;
    }

    // 高效分割（兼容CRLF/LF）
    const [head, body, foot] = [
        m3u8_text.slice(0, EXTINF_INDEX).trim(),
        m3u8_text.slice(EXTINF_INDEX, ENDLIST_INDEX).split(/\r?\n/g),
        m3u8_text.slice(ENDLIST_INDEX).trim()
    ];

    // 广告正则预处理
    let adRegex;
    if (ad_remove instanceof RegExp) {
        adRegex = ad_remove;
    } else if (typeof ad_remove === 'string') {
        adRegex = new RegExp(
            ad_remove.startsWith('reg:') ? ad_remove.slice(4) : ad_remove,
            'i'
        );
    } else {
        adRegex = /(advert|banner|promo|jump)/i; // 默认广告关键词
    }
    ...
    while (cursor < body.length) {
        ...
        if (line.startsWith('#EXTINF')) {
            const duration = parseFloat(line.match(/(\d+\.?\d*)/)[0]);
            const urlLine = body[cursor + 1];
            const isAd = adRegex.test(urlLine) || duration < 2; // 短于2秒视为广告
            !isAd && segments.push(line, urlLine);
            cursor += 2;
        } else if (line.startsWith('#EXT-X-DISCONTINUITY')) {
            const nextLine = body[cursor + 1];
            const isDisAd = adRegex.test(nextLine);
            !isDisAd && segments.push(line, ...body.slice(cursor + 1, cursor + 3));
            cursor += strictMode ? 3 : (nextLine.startsWith('#') ? 1 : 3);
        } else {
            strictMode ? cursor++ : segments.push(line);
        }
    }

    // 智能路径修复
    const baseURL = new URL(m3u8_url);
    const cleanBody = segments.map(line => 
        line.startsWith('http') ? line : 
        /^\//.test(line) ? new URL(line, baseURL.origin).href :
        new URL(line, baseURL).href
    ).join('\n');

    return [head, cleanBody, foot].join('\n');
}

// 示例：过滤包含"/ad/"路径且时长短的片段
const cleaned = fixAdM3u8Pro(null, 'https://example.com/playlist.m3u8', '/ad/');


/**
 * 智能对比去除广告增强版（基于原规则强化）
 * 支持：深度嵌套、多重广告模式、智能路径处理、高效过滤
 * @param m3u8_url m3u8播放地址
 * @param headers 自定义请求头
 * @returns {string} 净化后的m3u8内容
 */
function fixAdM3u8AiPro(m3u8_url, headers) {
    const startTime = Date.now();
    const config = {
        adThreshold: 2.5,    // 广告分段时长阈值(秒)
        minSegments: 3,       // 最小广告段数量
        patternCheckDepth: 15,// 模式检测深度
        nestingLimit: 5       // 最大嵌套深度
    };
    ...
    // 递归获取嵌套m3u8
    const fetchNestedM3u8 = (url, depth = 0) => {
        if (depth > config.nestingLimit) throw 'Exceeded nesting limit';
        
        const options = headers ? { headers } : {};
        let content = request(url, options)
            .trim()
            .split('\n')
            .map(line => line.startsWith('#') ? line : resolveUrl(url, line))
            .join('\n');

        // 检测嵌套m3u8
        const lastValidLine = content.split('\n')
            .filter(l => l.trim().length > 3)
            .reverse()
            .find(l => l.endsWith('.m3u8'));
        
        if (lastValidLine && lastValidLine !== url) {
            console.log(`发现嵌套层 ${depth+1}: ${lastValidLine}`);
            return fetchNestedM3u8(resolveUrl(url, lastValidLine), depth + 1);
        }
        return content;
    };

    // 智能广告模式检测
    const detectAdPatterns = (playlist) => {
        ...
        lines.forEach((line, index) => {
            ...
            if (line.startsWith('#EXTINF:')) {
                const duration = parseFloat(line.match(/\d+\.?\d*/)[0]);
                if (duration < config.adThreshold) adScore += 2;
                currentGroup.push({ index, duration });
            }

            // 广告标记检测
            if (line.includes('ADMARKER') || line.includes('CUE-OUT')) {
                adScore += 5;
                currentGroup.forEach(g => adLines.add(g.index));
            }

            // 不连续标记检测
            if (line.includes('DISCONTINUITY')) {
                adScore += 3;
                if (currentGroup.length >= config.minSegments) {
                    currentGroup.forEach(g => adLines.add(g.index));
                }
                currentGroup = [];
            }
            ...
            // 分组超时重置
            if (currentGroup.length > config.patternCheckDepth) {
                currentGroup = [];
                adScore = 0;
            }
        });
        ...
        const analyzeStructure = () => {
            ...
            lines.filter(l => !l.startsWith('#') && l.length > 10)
                .slice(0, config.patternCheckDepth)
                .forEach(url => {
                    const base = url.substring(0, url.lastIndexOf('/') + 1);
                    urlPatterns.set(base, (urlPatterns.get(base) || 0) + 1);
                });

            // 异常路径检测
            const mainPattern = [...urlPatterns.entries()].sort((a,b) => b[1]-a[1])[0][0];
            lines.forEach((line, index) => {
                if (!line.startsWith('#') && !line.includes(mainPattern)) {
                    adLines.add(index);
                    adLines.add(index-1); // 连带EXTINF行
                }
            });
        };
        analyzeStructure();
    ...
    try {
        // 主处理流程
        console.log(`[开始处理] ${m3u8_url}`);
        let m3u8Content = fetchNestedM3u8(m3u8_url);
        
        // 广告行检测
        const adLineNumbers = detectAdPatterns(m3u8Content);
        console.log(`检测到可疑广告行：${adLineNumbers.size}处`);

        // 生成净化内容
        const finalContent = m3u8Content.split('\n')
            .filter((line, index) => !adLineNumbers.has(index))
            .map(line => line.startsWith('#') ? line : resolveUrl(m3u8_url, line))
            .join('\n');
        ...
        return request(m3u8_url, headers ? { headers } : {});
    }
}

/* 核心增强特性：
1. 双重检测机制：实时特征扫描 + 后期结构分析
2. 智能路径分析：自动识别主资源路径模式
3. 动态阈值控制：基于广告时长和分段数量
4. 深度嵌套支持：安全递归处理多层嵌套
5. 高效过滤算法：使用Set快速处理行号
6. 错误防御机制：异常捕获和回退策略
*/


/**
 * 终极智能广告过滤系统 - 支持深度学习特征识别与动态模式学习
 * @param {string} m3u8_url - 源地址
 * @param {Object} [headers] - 请求头
 * @param {Object} [ai_model] - 可选的预训练模型 
 * @returns {string} 净化后的m3u8
 */
function aiM3u8Purifier(m3u8_url, headers, ai_model) {
    const AI_THRESHOLD = 0.82; // 深度学习置信度阈值
    const AD_PATTERNS = { // 多CDN广告特征库
        ff: {duration: [1,3], marker: 'discontinuity'},
        kk: {params: ['token','expire'], path: /\/ads?\//},
        qy: {filename: /^\d{6}_ad/}
    };

    // 深度特征分析器 (模拟轻量级CNN)
    class FeatureExtractor {
        static analyze(segment) {
            // 特征维度: [时长, 路径深度, 参数数量, 文件名熵值]
            const pathDepth = segment.url.pathname.split('/').length;
            const paramCount = [...segment.url.searchParams.keys()].length;
            const entropy = this.calcEntropy(segment.url.pathname.split('/').pop());
            return [segment.duration, pathDepth, paramCount, entropy];
        }

        static calcEntropy(str) {
            if (!str) return 0;
            const freq = {};
            for (const c of str) freq[c] = (freq[c] || 0) + 1;
            return -Object.values(freq).reduce((a,v) => a + (v/str.length)*Math.log2(v/str.length),0);
        }
    }
    ...
    const ruleEngine = {
        apply: (segments) => {
            return segments.map(seg => {
                ...
                // CDN特征匹配
                for (const [cdn, rule] of Object.entries(AD_PATTERNS)) {
                    if (rule.duration && seg.duration >= rule.duration[0] && seg.duration <= rule.duration[1]) score += 0.3;
                    if (rule.path && rule.path.test(seg.url.pathname)) score += 0.4;
                    if (rule.params && rule.params.every(p => seg.url.searchParams.has(p))) score += 0.5;
                    if (rule.filename && rule.filename.test(seg.url.pathname.split('/').pop())) score += 0.6;
                }

                // AI模型预测
                if (ai_model) {
                    const features = FeatureExtractor.analyze(seg);
                    score += ai_model.predict(features) * 0.7;
                }

                return {...seg, adScore: score};
            });
        }
    };

    // 增强型递归加载器 (支持302追踪)
    async function loadM3u8(url, redirectCount=0) {
        if (redirectCount > 3) throw 'Too many redirects';
        
        const res = await fetch(url, {headers, redirect: 'manual'});
        if (res.status === 302) {
            return loadM3u8(res.headers.get('Location'), redirectCount+1);
        }

        let content = await res.text();
        const baseURL = new URL(url);
        const lines = content.split('\n').map(line => {
            if (!line.startsWith('#')) return new URL(line, baseURL).href;
            return line.replace(/URI="(.*?)"/g, (m, p1) => {
                try { return `URI="${new URL(p1, baseURL).href}"` } 
                catch { return m }
            });
        });

        // 深度嵌套检测
        const masterLine = lines.find(line => line.includes('EXT-X-STREAM-INF'));
        return masterLine ? loadM3u8(new URL(masterLine.split('\n')[0], baseURL)) : lines.join('\n');
    }

    // 执行流程
    try {
        let content = await loadM3u8(m3u8_url);
        const segments = parseSegments(content);
        const analyzed = ruleEngine.apply(segments);
        
        // 动态阈值计算
        const scores = analyzed.map(s => s.adScore);
        const dynamicThreshold = Math.min(AI_THRESHOLD, 
            mean(scores) + 1.5 * stdDev(scores)
        );
        ...
        content.split('\n').forEach(line => {
            ...
            const seg = analyzed.find(s => s.lines.includes(line));
            if (seg && seg.adScore >= dynamicThreshold) {
                skipLines = seg.lines.length - 1;
            } else {
        ...
        // 学习反馈机制
        if (ai_model) {
            const trainData = analyzed.map(s => ({
                features: FeatureExtractor.analyze(s),
                label: s.adScore > dynamicThreshold ? 1 : 0
            }));
            ai_model.onlineTrain(trainData);
        }

        return cleanLines.join('\n');
    } catch (e) {
        console.error('AI处理失败,返回原始内容:', e);
        return await (await fetch(m3u8_url, {headers})).text();
    }
